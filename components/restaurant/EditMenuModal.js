...
export default function EditMenuModal({
  ...
  // Initialize edit menu when menu prop changes
  useEffect(() => {
    if (menu) {
      setEditMenu({
        name: menu.name || "",
        ...
        images: menu.images || [],
        tags: menu.tags || [],
        validFrom: menu.validFrom ? menu.validFrom.split("T")[0] : "",
        validUntil: menu.validUntil ? menu.validUntil.split("T")[0] : "",
  ...
  useEffect(() => {
    if (editMenu.isAutoPricing && availableItems.length > 0) {
      const calculatedPrice = calculatePriceFromItems(editMenu.items);
      setEditMenu((prev) => ({
        ...prev,
        price: calculatedPrice.toFixed(2),
      }));
    }
  }, [editMenu.items, editMenu.isAutoPricing, availableItems]);
  ...
  const addItemToMenu = (itemId) => {
    ...
    if (editMenu.items.find((item) => item.itemId === numericItemId)) {
  ...
  const addTag = () => {
    if (newTag.trim() && editMenu.tags.length < 10 && newTag.length <= 30) {
      if (!editMenu.tags.includes(newTag.trim())) {
        setEditMenu((prev) => ({
          ...prev,
  ...
  const addImage = () => {
    if (newImage.trim() && editMenu.images.length < 5) {
      try {
        new URL(newImage.trim()); // Validate URL
        if (!editMenu.images.includes(newImage.trim())) {
          setEditMenu((prev) => ({
            ...prev,
  ...
  const handleUpdateMenu = async () => {
    ...
    try {
      ...
      const response = await restaurantAPI.updateMenu(menu.id, menuData);
      onMenuUpdated(response.menu);
      closeModal();
    } catch (error) {
  ...
  return (
    <ModalWrapper
      title="Edit Menu"
      subtitle={`Modify details for "${menu.name}"`}
      onClose={closeModal}
    >
      <div className="space-y-6">
        ...
        <FormField label="Description" error={fieldErrors.description}>
          <textarea
            ...
            onChange={(e) => updateEditMenuField("description", e.target.value)}
        ...
        <FormField label="Menu Items" error={fieldErrors.items}>
          <div className="space-y-4">
            ...
            {!itemsLoading && availableItems.length > 0 && (
              <div className="flex gap-2">
                <select
                  ...
                  disabled={editMenu.items.length >= 20}
                >
            ...
            {editMenu.items.length > 0 && (
              <div className="border rounded-lg p-4 space-y-3">
                <h4 className="font-medium text-gray-900">
                  Selected Items ({editMenu.items.length})
                </h4>
                ...
                {editMenu.items.map((menuItem, index) => {
                  const itemDetails = getItemDetails(menuItem.itemId);
                  return (
                    <div
                      key={menuItem.itemId}
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            Base price: $
                            {itemDetails.price?.toFixed(2) || "0.00"}
                          </div>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="flex items-end">
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              checked={menuItem.isOptional || false}
                              onChange={(e) =>
                                updateMenuItem(
                                  menuItem.itemId,
                                  "isOptional",
                ...
                {/* Price Calculation Summary for Edit */}
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-4">
                  <h5 className="text-sm font-medium text-blue-900 mb-2">
                    Price Calculation
                  </h5>
                  <div className="space-y-1 text-xs text-blue-800">
                    {editMenu.items.map((menuItem) => {
                      const itemDetails = getItemDetails(menuItem.itemId);
                    ...
                    <div className="border-t border-blue-300 pt-2 mt-2">
                      <div className="flex justify-between font-medium">
                        ...
                        <span>
                          ${calculatePriceFromItems(editMenu.items).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {/* Validation Message */}
            {fieldErrors.items && (
              <p className="text-sm text-red-600 mt-1">{fieldErrors.items}</p>
            )}
          </div>
        </FormField>

        {/* Pricing Options for Edit */}
        <div className="space-y-4">
          {/* Auto-pricing toggle */}
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={editMenu.isAutoPricing}
                onChange={(e) =>
                  updateEditMenuField("isAutoPricing", e.target.checked)
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField
              label="Price"
              required={!editMenu.isAutoPricing}
            ...
            >
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => updateEditMenuField("price", e.target.value)}
                  className={`form-input pl-8 ${
                    editMenu.isAutoPricing
                      ? "bg-gray-100 cursor-not-allowed"
                      : ""
                  }`}
                  placeholder="0.00"
                  disabled={editMenu.isAutoPricing}
                  readOnly={editMenu.isAutoPricing}
                />
              </div>
              {editMenu.isAutoPricing && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField label="Valid From" error={fieldErrors.validFrom}>
            <input
              ...
              onChange={(e) => updateEditMenuField("validFrom", e.target.value)}
              className="form-input"
            />
          </FormField>
        ...
        <FormField label="Sort Order" error={fieldErrors.sortOrder}>
          <input
            ...
            onChange={(e) => updateEditMenuField("sortOrder", e.target.value)}
            className="form-input"
            placeholder="0"
          />
        </FormField>
      ...
      {process.env.NODE_ENV === "development" && createError && (
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <p className="text-xs text-yellow-700 mt-1">Error: {createError}</p>
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          disabled={
            actionLoading ||
            !editMenu.name.trim() ||
            (!editMenu.isAutoPricing && !editMenu.price)
          }
